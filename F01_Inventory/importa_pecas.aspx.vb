...
Public Class importa_pecas
    ...
    Protected Sub botao_importar_dados_Click(sender As Object, e As EventArgs) Handles botao_importar_dados.Click
        ...
        atualiza_estoque(tabela_temporaria, atualizacoes, insercoes)
        ...
        'MOSTRA MSGBOX
        Dim meuscript As String = "<script>alert('Success! " + CStr(atualizacoes) + " updates and " + CStr(insercoes) + " new records');</script>"
    ...
    'rotina para inserir novos itens ou atualizar existentes em ESTOQUE
    Public Sub atualiza_estoque(ByVal tabela_temporaria As DataSet, ByRef atualizacoes As Integer, ByRef insercoes As Integer)
        ...
        Dim SQL As String = "SELECT * FROM [ESTOQUE] WHERE([CÓDIGO_FABRICANTE] = @cod_fabricante)"
        ...
        For i As Integer = 0 To (cont_linhas - 1) Step 1
            ...
            Dim fabricante As String = ds.Tables("tabela").Rows(i).ItemArray.GetValue(1).ToString()
            Dim cod_fabricante As String = ds.Tables("tabela").Rows(i).ItemArray.GetValue(2).ToString()
            Dim descricao As String = ds.Tables("tabela").Rows(i).ItemArray.GetValue(3).ToString()
            ...
            'se nao foi localizado registro existente em ESTOQUE, e descrição do item importado nao for vazio, entao insiro na tabela 
            If ds1.Tables("tabela1").Rows.Count = 0 And (ds.Tables("tabela").Rows(i).ItemArray.GetValue(2).ToString() <> "") Then

                Dim SQL_1 As String = "INSERT INTO [ESTOQUE] ([FABRICANTE], [CÓDIGO_FABRICANTE], [DESCRIÇÃO], [RAZAO])" _
                         + "VALUES (@fabricante, @cod_fabricante, @descrica, @razao)"
                'executa o comando
                ...
                comando.Parameters.Add(New OleDbParameter("@razao", "N/D"))
                comando.ExecuteNonQuery()
